...
HarvardX - Data Science textbook by prof. Rafael Izarri

## Text Mining

This page corresponds to the textbook section on text mining External link: https://rafalab.github.io/dsbook/text-mining.html
...
`r android_iphone_or %>% filter(Android+iPhone > 100) %>% arrange(or)`
...
`r get_sentiments("afinn")`
# The loughran and nrc lexicons provide several different sentiments:

`r get_sentiments("loughran") %>% count(sentiment)`

`r get_sentiments("nrc") %>% count(sentiment)`
...
tweet_words %>% inner_join(nrc, by = "word") %>% 
  select(source, word, sentiment) %>% sample_n(15)
```
...
`r tweet_words %>% group_by(source) %>% summarize(n = n())`
